...
@Injectable()
export class ProcesadorUbicacionService {
  private readonly logger = new Logger(ProcesadorUbicacionService.name);
    ...
    try {
      ...
      for (const datosRuta of datosRutas) {
        ...
        if (yaExiste) {
          ...
          this.logger.debug(`Ubicación duplicada ignorada para alerta ${uuidAlerta}: ${latitud}, ${longitud} en ${fechaHora}`);
          continue;
        }
